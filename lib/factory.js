...
Factory.prototype.createJob =  function createJob(req, res, context, next) {
  ...
  try {
    ...
    if(originalJob) {
      winston.log('debug', 'Coalescing Request!');
      winston.log('debug', 'Sending response from ' + originalJob.id);
...
Factory.prototype.coalesceRequests = function coalesceRequests(res, context) {
  ...
  function checkIfDuplicate(jobs) {
    for (var j = 0; j < jobs.length; j++) {
      var otherJob = jobs[j];
      winston.log('debug', 'ThisContext: ' + JSON.stringify(context));
      winston.log('debug', 'OtherContext: ' + JSON.stringify(otherJob.context));
  ...
  winston.log('debug', 'Attempting to coalesce');
  ...
  winston.log('debug', 'OriginalRunningJob: ' + originalJob);
  if(originalJob) {
    winston.log('debug', 'Attatching response to ' + originalJob.id);
    return originalJob;
  }
  ...
  winston.log('debug', 'OriginaCoalescelJob: ' + originalJob);
  if(originalJob) {
    return originalJob;
  }
