...
function Factory(config) {
  ...
  this.jobQueue = async.queue(function(job, callback) {
    winston.log('debug', 'Job ' + job.id + ' added to queue');

    // Start building the image when a job is added to queue
    job.start(function (err) {
      winston.log('debug', 'Finished job ' + job.id);
      callback(err);
      var isSuccessful = (err ? false : true);
      var image = this.getImage(job);
      //Notify build was successful / failed.
      var notifyCtx = _.extend({}, this.context, {
        baseUrl: this.config.baseUrl,
        env: this.config.env,
        id: job.id
      });
      _.forEach(this.notifiers, function(notifier) {
        notifier.notify(isSuccessful, notifyCtx, image);
      });
      
    });

  }, 1); // Limit the number of concurrent jobs to 2

  // Say something when all the jobs are finished
  this.jobQueue.drain = function() {
    winston.log('debug', 'All jobs have finished');
  };
...
Factory.prototype.newJob = function newJob(context) {
  ...
  _this.jobQueue.push(job, function() {
