...
Factory.prototype.newJob = function newJob(context) {
  ...
  // If job with equal context was created < x minutes ago,
  // don't do anything with it
  var THIRTY_SECONDS = 30 * 1000;
  var isDuplicate = _this.jobs.isDuplicate(job, THIRTY_SECONDS);
  if(isDuplicate) {
    job.status = constants.EVENTS.STATUS_DUPLICATE;
      winston.log('debug', 'Job: ' + job.id + ' is a duplicate. Not running.');
  }
