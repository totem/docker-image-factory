...
module.exports = {
    ...
    if (options.timeout) {
      timeoutInterval = setInterval(
        function () {
          ...
          step.kill('SIGTERM');
          writeToLog(logFile, '*** Process timed out after ' + options.timeout + 'ms, terminating! ***');
  ...
  writeToLog: function (logFile, stuffToWrite) {
    writeToLog(logFile, stuffToWrite);
  }
};

function writeToLog(logFile, stuffToWrite) {
  // Open the log for appending
  var log = fs.createWriteStream(logFile, { flags: 'a' });
  log.write(stuffToWrite);
  log.end(); // Close the stream
}
